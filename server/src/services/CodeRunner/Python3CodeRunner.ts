...
export class Python3CodeRunner extends CodeRunner {
  private dockerImageName = sandboxes.python3;
  constructor(
    timeLimit: number,
    memoryLimit: number,
    cpus: number,
  ...
  protected getRunCommand(inputPath: string, outputPath: string): string {
    const command = ` \
      ...
      sh -c "time -v timeout ${this.timeLimit} python3 ${this.sandboxProgPath} < ${this.sandboxInputPath}" \
      > ${outputPath}
      EXIT_CODE=$?
